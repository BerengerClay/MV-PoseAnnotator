import os
import numpy as np
from PyQt6.QtWidgets import QGraphicsView, QGraphicsScene, QPushButton, QLabel, QMessageBox, QMenu, QStyle
...
from src.constants import COCO_KEYPOINTS, COCO_SKELETON, CAMERA_KEYS
from src.items import KeypointItem, SkeletonItem, BBoxItem, ReprojectedPointItem, DiscrepancyLineItem
...
class CameraWidget(QGraphicsView):
    ...
    def __init__(self, camera_id, camera_name, main_win):
        ...
        self.name_label.setStyleSheet("color: #94a3b8; background-color: rgba(15, 23, 42, 180); padding: 4px 8px; border-radius: 4px; font-weight: bold;")
    ...
    def resizeEvent(self, event):
        ...
        if hasattr(self, 'delete_ann_btn') and self.delete_ann_btn:
            ...
            offset_x = (w_toggle + 16) if (hasattr(self, 'toggle_view_btn') and not self.toggle_view_btn.isHidden()) else 10
    ...
    def mousePressEvent(self, event):
        ...
        if event.button() == Qt.MouseButton.LeftButton and event.modifiers() == Qt.KeyboardModifier.ShiftModifier:
            ...
            self.bbox_item = self.scene.addRect(QRectF(self._bbox_start_scene, self._bbox_start_scene), 
                                                QPen(QColor(234, 179, 8), 2, Qt.PenStyle.DashLine)) # Yellow dash
            event.accept()
    ...
    def load_frame(self, img_path, annotation, preserve_view=False):
        ...
        is_same_image = (hasattr(self, 'current_img_path') and 
                         self.current_img_path == img_path and 
        ...
        if not os.path.exists(img_path):
            txt_item = self.scene.addText(f"Image not found:\n{os.path.basename(img_path)}")
        ...
        has_keypoints = keypoints and any(keypoints[idx*3 + 2] > 0 for idx in range(17))
        ...
        if keypoints:
            ...
            for idx in range(17):
                ...
                if offset + 2 < len(keypoints):
                    kx, ky, kv = keypoints[offset], keypoints[offset+1], keypoints[offset+2]
                    if kv > 0:
                        ...
                        if not np.isnan(X_3d[0]):
                            ...
                            if x_proj[2] != 0:
                                ...
                                if 0.0 <= u_proj <= 1920.0 and 0.0 <= v_proj <= 1080.0:
                                    proj_item = ReprojectedPointItem(u_proj, v_proj, kp_idx, COCO_KEYPOINTS[kp_idx], self)
                                    ...
                                    if kp_idx in self.keypoint_items:
                                        ...
                                        line_item = DiscrepancyLineItem(kp_item.pos().x(), kp_item.pos().y(), u_proj, v_proj)
    ...
    def apply_bbox_view(self):
        ...
        bbox = self.current_annotation.get("bbox", [0, 0, 0, 0]) if self.current_annotation else [0, 0, 0, 0]
        keypoints = self.current_annotation.get("keypoints", []) if self.current_annotation else []
        ...
        if bbox and sum(bbox) > 0:
            ...
            padding_x = w * 0.2
            padding_y = h * 0.2
            padded_rect = QRectF(x - padding_x, y - padding_y,
                                 w + 2 * padding_x, h + 2 * padding_y)
    ...
    def update_keypoint_pos(self, point_id, x, y, save_and_sync=False):
        ...
        self.main_win.update_keypoint(self.camera_id, point_id, x, y, save_and_sync=save_and_sync)
        ...
        if not save_and_sync and getattr(self.main_win, 'realtime_triangulation_enabled', False):
    ...
    def show_insert_keypoint_menu(self):
        ...
        actions = {}
        if not has_bbox:
            action = menu.addAction("Add BBox")
            ...
            if not missing_kps:
                self.main_win.status_bar.showMessage("All keypoints are already present on this view.", 3000)
                return
        ...
        if selected_action in actions:
            ...
            if val == "add_bbox":
                ...
                ann["keypoints"][offset+2] = 2  # Visibility 2 (manual adjustment/confirmed)
                ann["num_keypoints"] = sum(1 for k in range(17) if ann["keypoints"][k*3 + 2] > 0)
    ...
    def swap_left_right_keypoints(self):
        ...
        for l_idx, r_idx in pairs:
            ...
            keypoints[l_off+1], keypoints[r_off+1] = keypoints[r_off+1], keypoints[l_off+1]
            keypoints[l_off+2], keypoints[r_off+2] = keypoints[r_off+2], keypoints[l_off+2]
